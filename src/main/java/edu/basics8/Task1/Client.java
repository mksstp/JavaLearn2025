...
public class Client {
    ...
    private final InetSocketAddress hostAdress;

    public Client(String host, int port) {
        this.hostAdress = new InetSocketAddress(host, port);
    }

    @SuppressWarnings("checkstyle:RegexpSinglelineJava")
    public void connect(String messageToSend) {
        try (SocketChannel server = SocketChannel.open(hostAdress)) {
